 ...
 */
public class ResponseTimeStatsVolatileImpl extends AbstractVolatileStats implements ResponseTimeStats {
    ...
    private volatile long totalExclusive;
    private volatile long minValue;
    private volatile long maxValue;
    private volatile double sumOfSquares;

    protected ResponseTimeStatsVolatileImpl() {
        super();
    }

    @Override
    public Object clone() throws CloneNotSupportedException {
        ResponseTimeStatsVolatileImpl newStats = new ResponseTimeStatsVolatileImpl();
    ...
    public final void merge(StatsBase statsObj) {
        if (statsObj instanceof ResponseTimeStatsVolatileImpl) {
            ResponseTimeStatsVolatileImpl stats = (ResponseTimeStatsVolatileImpl) statsObj;
            if (stats.count > 0) {
                if (count > 0) {
