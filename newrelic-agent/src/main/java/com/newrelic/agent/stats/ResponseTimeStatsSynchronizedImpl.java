 ...
 */
public class ResponseTimeStatsSynchronizedImpl extends AbstractStats implements ResponseTimeStats {
    ...
    protected ResponseTimeStatsSynchronizedImpl() {
        super();
    }

    @Override
    public Object clone() throws CloneNotSupportedException {
        ResponseTimeStatsSynchronizedImpl newStats = new ResponseTimeStatsSynchronizedImpl();
    ...
    public final void merge(StatsBase statsObj) {
        if (statsObj instanceof ResponseTimeStatsSynchronizedImpl) {
            ResponseTimeStatsSynchronizedImpl stats = (ResponseTimeStatsSynchronizedImpl) statsObj;
            synchronized (lock) {
                if (stats.count > 0) {
                    if (count > 0) {
